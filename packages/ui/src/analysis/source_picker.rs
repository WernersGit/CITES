...
pub fn SourcePicker(props: SourcePickerProps) -> Element {
    ...
    // list archvies on the connected node
    let on_browse_node = {
        move |_| {
    ...
    //TODO: merge with ui/source_picker.rs to avoid duplciation
    ...
    let content = match state.read().clone() {
        PickerState::SelectSource => rsx! {
            p {
                style: "margin: 0 0 1.5rem; color: #555; font-size: 0.9rem;",
        ...
        PickerState::NodeList { files } | PickerState::ClientList { files } => {
            ...
            rsx! {
                p {
                    style: "margin: 0 0 0.75rem; font-size: 0.85rem; color: #555;",
                    if is_node { "Recordings stored on the ndoe:" } else { "Locally stored recordings:" }
                }
                if files.is_empty() {
                        ...
                        file_row {
                            ...
                            on_select: {
                                ...
                                move |_| {
                                    ...
                                    let cb = on_loaded.clone();
                                    let mut st = state.clone();
                                    spawn(async move {
                                        *st.write() = PickerState::Loading(format!("Loading {}…", fname));
                                        let result = if is_node {
                                            connection.fetch_archive_file(&fname).await
                                        } else {
                                            let path = std::path::Path::new("./captures").join(&fname);
                                            std::fs::read(&path).map_err(|e| e.to_string())
                                        };
                                        match result {
                                            Ok(data) => match PcapParser::parse_bytes(&data) {
                                                Ok(pkts) => cb.call(pkts),
                                                Err(e) => *st.write() = PickerState::Error(e.to_string()),
                                            },
                                            Err(e) => *st.write() = PickerState::Error(e),
                                        }
                                    });
                ...
                button {
                    ...
                    onclick: move |_| { *state.write() = PickerState::SelectSource; },
                    "← Back"
                }
            }
        },
        ...
        div {
            ...
            div {
                ...
                div { style: "display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.25rem;",
                    h2 { style: "margin: 0; font-size: 1.2rem;", "Select data source" }
                    button {
