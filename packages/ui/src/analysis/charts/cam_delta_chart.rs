use dioxus::prelude::*;
use super::{ClockDriftSample, COLORS, fmt_hms};

#[derive(Props, Clone, PartialEq)]
pub struct CamDeltaChartProps {
...
/// Scatter plot of CAM generationDeltaTime over time — the sawtooth pattern.
/// Matches Python CamAnalyzer.py report 4 "CAM Generation Delta Time".
/// X = capture timestamp, Y = generationDeltaTime (ms, 0–65535).
#[component]
pub fn CamDeltaChart(props: CamDeltaChartProps) -> Element {
    if props.samples.is_empty() {
        return rsx! { p { style: "color: #999; font-size: 0.85rem;", "No generationDeltaTime data available (requires decoded CAM payload)." } };
    }
    ...
    rsx! {
        svg {
            ...
            for (mi, mac) in macs_with_data.iter().enumerate() {
                {
                    ...
                    let pts: Vec<&ClockDriftSample> = props.samples.iter()
                        .filter(|s| &s.mac == *mac)
                        .step_by(stride)
                        .collect();
                    rsx! {
                        for s in pts.iter() {
                            circle {
                                key: "cd{s.capture_ms}{mi}",
                                cx: "{xf(s.capture_ms)}", cy: "{yf(s.gen_delta_ms as f64)}",
            ...
            text { x: "10", y: "{tm + ph / 2.0}", "text-anchor": "middle", "font-size": "10", fill: "#666",
                "transform": "rotate(-90,10,{tm + ph / 2.0})", "generationDeltaTime (ms)"
            }
            ...
            for (mi, mac) in macs_with_data.iter().take(8).enumerate() {
                circle { key: "lr{mi}", cx: "{lm + mi as f64 * 65.0 + 4.0}", cy: "{h - 6.0}", r: "4", fill: "{COLORS[mi % COLORS.len()]}", opacity: "0.85" }
                text { key: "lt{mi}", x: "{lm + mi as f64 * 65.0 + 12.0}", y: "{h - 2.0}", "font-size": "8", "font-family": "monospace", fill: "#444", "{mac}" }
            }
        }
    }
}
